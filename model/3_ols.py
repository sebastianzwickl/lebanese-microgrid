...
# Modell mit nur signifikanten Variablen erneut schätzen
significant_vars = model.pvalues[model.pvalues < 0.1].index.tolist()
if 'const' in significant_vars:
    significant_vars.remove('const')

X_significant = input_data_clean[significant_vars]
X_significant = sm.add_constant(X_significant)

model_significant = sm.OLS(y, X_significant).fit()
print("\n+ REVISED MODEL WITH SIGNIFICANT VARIABLES ONLY:")
print(model_significant.summary())

# Neue Preiselastizität nach Selektion signifikante Variablen
if 'LN_Selling Price [$/kWh]' in model_significant.params:
    new_elasticity = model_significant.params['LN_Selling Price [$/kWh]']
    print("\n+ UPDATED ELECTRICITY DEMAND ELASTICITY: {:.4f}".format(new_elasticity))
else:
    print("\n+ Selling Price is NOT significant in the revised model.")
